...
export default function PipelineOverview({ leads }) {
  ...
  const counts = leads.reduce((acc, lead) => {
    ...
    acc[lead.status].value += Number(lead.dealValue || lead.value || 0);
    return acc;
  }, {});

  const stages = [
    { label: 'New', display: 'New', color: 'bg-blue-600', text: 'text-blue-600 dark:text-blue-400' },
    { label: 'Contacted', display: 'Contacted', color: 'bg-indigo-600', text: 'text-indigo-600 dark:text-indigo-400' },
    { label: 'Meeting Scheduled', display: 'Meeting', color: 'bg-amber-500', text: 'text-amber-500 dark:text-amber-400' },
    { label: 'Proposal Sent', display: 'Proposal', color: 'bg-purple-700', text: 'text-purple-700 dark:text-purple-400' },
    { label: 'Won', display: 'Won', color: 'bg-emerald-500', text: 'text-emerald-600 dark:text-emerald-400' },
    { label: 'Lost', display: 'Lost', color: 'bg-rose-500', text: 'text-rose-600 dark:text-rose-400' },
  ];
        ...
        {total > 0 ? stages.map((stage) => {
          ...
          return (
            <div
              ...
              style={{ width }}
              title={`${stage.display}: ${count} lead${count !== 1 ? 's' : ''}`}
            />
          );
        }) : (
          ...
          return (
            ...
            >
              <div className="mb-2 flex items-center gap-2">
                ...
                <p className="truncate text-sm font-semibold text-gray-800 dark:text-gray-200">{stage.display}</p>
              </div>
